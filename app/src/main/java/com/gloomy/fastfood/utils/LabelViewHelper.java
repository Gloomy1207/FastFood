...
public final class LabelViewHelper {
    ...
    public void onDraw(Canvas canvas, int measuredWidth, int measuredHeight) {
        ...
        float beginWOffset = (1.4142135f * actualDistance) / 2 - textBound.width() / 2;
        if (beginWOffset < 0) beginWOffset = 0;

        canvas.drawTextOnPath(text, textPath, beginWOffset, textBound.height() / 2, textPaint);
    }
    ...
    private void calcOffset(int measuredWidth, int measuredHeight) {
        ...
        switch (orientation) {
            ...
            case 2: // RIGHT_TOP
                ...
                rectPath.lineTo((float) measuredWidth, distance);
                rectPath.lineTo((float) measuredWidth, distance + height);
                rectPath.close();

                textPath.reset();
                textPath.moveTo(startPosX + middle, 0);
                textPath.lineTo((float) measuredWidth, distance + middle);
                textPath.close();
            ...
            case 3: // LEFT_BOTTOM
                ...
                rectPath.lineTo(distance + height, (float) measuredHeight);
                rectPath.lineTo(distance, (float) measuredHeight);
                rectPath.lineTo(0, startPosY + height);
                rectPath.close();

                textPath.reset();
                textPath.moveTo(0, startPosY + middle);
                textPath.lineTo(distance + middle, (float) measuredHeight);
                textPath.close();
            ...
            case 4: // RIGHT_BOTTOM
                ...
                rectPath.reset();
                rectPath.moveTo(startPosX, (float) measuredHeight);
                rectPath.lineTo(measuredWidth, startPosY);
                rectPath.lineTo(measuredWidth, startPosY + height);
                rectPath.lineTo(startPosX + height, (float) measuredHeight);
                rectPath.close();

                textPath.reset();
                textPath.moveTo(startPosX + middle, (float) measuredHeight);
                textPath.lineTo((float) measuredWidth, startPosY + middle);
                textPath.close();
